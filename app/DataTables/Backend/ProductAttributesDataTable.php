...
class ProductAttributesDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return $dt
            ->addIndexColumn()
            ->addColumn('action', function (ProductAttribute $row) {
                $actions = [
                    [
                        'type' => 'link',
                        'label' => 'Edit',
                        'icon' => 'bi-pencil-square',
                        'url' => route('admin.products.attributes.edit', $row->slug),
                    ],
                    ['type' => 'divider'],
                    [
                        'type' => 'link',
                        'label' => 'Show',
                        'icon' => 'bi-eye',
                        'url' => route('admin.products.attributes.show', $row->slug),
                    ],
                    ['type' => 'divider'],
                    [
                        'type' => 'delete',
                        'label' => 'Delete',
                        'icon' => 'bi-trash',
                        'url' => route('admin.products.attributes.destroy', $row->slug),
            ...
            ->editColumn('created_at', function (ProductAttribute $row) {
                return $row->created_at ? $row->created_at->format('d M Y H:i') : '';
            })
            ->editColumn('updated_at', function (ProductAttribute $row) {
                return $row->updated_at ? $row->updated_at->format('d M Y H:i') : '';
            })
            ->editColumn('is_active', function (ProductAttribute $row) {
    ...
    {
        return $model->newQuery()->select('product_attributes.*');
    }
    ...
    {
        return 'ProductAttributes_'.date('YmdHis');
    }
}
