...
@push('backend_styles')
    ...
    <style>
        ...
        .img-thumb { max-width: 220px; max-height: 220px; object-fit: cover; border-radius: .25rem; }
        .meta-label { font-weight: 600; color: #495057; }
        .meta-value { color: #212529; }
        .card-footer .btn + .btn { margin-left: .5rem; }
        .badge-blank { background: #f8f9fa; color: #6c757d; border: 1px solid #e9ecef; }
        pre.json { background:#f8f9fa; padding:0.5rem; border-radius:4px; font-size:.9rem; }
    </style>
@endpush
        ...
        <div class="container-fluid">
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <div class="row gx-4 gy-4">
                        ...
                        <div class="col-lg-4">
                            <div class="mb-3 text-center">
                                @if(!empty($product->images))
                                    <img src="{{ asset($product->images) }}" alt="Main image" class="img-thumb mb-2">
                                @else
                                    <div class="img-thumb d-flex align-items-center justify-content-center badge-blank mb-2">
                                        No Image
                                    </div>
                                @endif
                                ...
                                <div class="d-flex flex-wrap gap-2">
                                    @if(isset($product->images) && $product->images->count() > 0)
                                        @foreach($product->images as $img)
                                            <div>
                                                <img src="{{ asset($img->path ?? $img->url ?? '') }}" alt="gallery"
                            ...
                            <div>
                                ...
                                <p class="mb-1"><span class="meta-label">SKU:</span> <span class="meta-value">{{ $product->sku ?? '—' }}</span></p>
                                <p class="mb-1"><span class="meta-label">Slug:</span> <span class="meta-value">{{ $product->slug ?? '—' }}</span></p>
                                <p class="mb-1"><span class="meta-label">Status:</span> <span class="meta-value">{{ ucfirst($product->status ?? '—') }}</span></p>
                                <p class="mb-1"><span class="meta-label">Featured:</span> <span class="meta-value">{{ $product->is_featured ? 'Yes' : 'No' }}</span></p>
                                <p class="mb-0"><span class="meta-label">Type:</span> <span class="meta-value">{{ ucfirst(str_replace('_',' ',$product->product_type ?? '—')) }}</span></p>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-8">
                            ...
                            <div class="card card-light card-outline mb-3">
                                ...
                                <div class="card-body">
                                    <p class="mb-2"><span class="meta-label">Name:</span> <span class="meta-value">{{ $product->name ?? '—' }}</span></p>
                                ...
                                <div class="card-body">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <div class="meta-value">{{ ucfirst($product->allow_backorders ?? 'no') }}</div>
                                        </div>
                    ...
                    <div class="card card-secondary card-outline mb-3 simple-section">
                        <div class="card-header">
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                <div class="col-md-3">
                                    <p class="mb-0"><span class="meta-label">Regular Price</span></p>
                                    <div class="meta-value">{{ $product->price ? number_format($product->price, 2) : '—' }}</div>
                                </div>
                                <div class="col-md-3">
                                    <p class="mb-0"><span class="meta-label">Sale Price</span></p>
                                    <div class="meta-value">{{ $product->sale_price ? number_format($product->sale_price, 2) : '—' }}</div>
                                </div>
                                <div class="col-md-3">
                                    <p class="mb-0"><span class="meta-label">Stock Quantity</span></p>
                                    <div class="meta-value">{{ $product->stock_quantity ?? '—' }}</div>
                                </div>

                                <div class="col-md-3">
                                    <p class="mb-0"><span class="meta-label">Low Stock Threshold</span></p>
                                    <div class="meta-value">{{ $product->low_stock_threshold ?? '—' }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        @php
                                                            ...
                                                            $attrTexts = [];
                                                            if(isset($variant->attributeValues) && $variant->attributeValues->count()){
                                                                foreach($variant->attributeValues as $av){
                                                                    $attrTexts[] = (optional($av->attribute)->name ? optional($av->attribute)->name . ': ' : '') . ($av->value ?? $av->display_value ?? $av->name ?? '—');
                                                                }
                                                            } elseif(isset($variant->attributes) && $variant->attributes->count()){
                                                                foreach($variant->attributes as $a){
                                                                    $attrTexts[] = $a->name . ': ' . ($a->pivot->value ?? $a->value ?? '—');
                                                                }
                                                            }
                                                        @endphp
                                                    ...
                                                    <td>
                                                        @if(!empty($variant->main_image))
                                                            <img src="{{ asset($variant->main_image) }}" alt="vimg" style="width:70px;height:70px;object-fit:cover;">
                                                        @else
                ...
                <div class="card-footer d-flex align-items-center">
                    ...
                    <div class="ms-auto">
                        <a href="{{ route('shop.products.edit', $product->id) }}" class="btn btn-primary"><i class="bi bi-pencil-square"></i> Edit</a>
                        <form action="{{ route('shop.products.destroy', $product->id) }}" method="POST" class="d-inline-block" onsubmit="return confirm('Delete this product?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger ms-2"><i class="bi bi-trash"></i> Delete</button>
                        </form>
                    </div>
                </div>
            </div>
...
@push('backend_scripts')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            tipList.map(function (el) { return new bootstrap.Tooltip(el); });
        });
    </script>
@endpush
